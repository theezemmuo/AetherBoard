...
import { ArrowCounterClockwise, ClipboardText, Sparkle } from 'phosphor-react';
import { useNavigate } from 'react-router-dom';

export function Home() {
    // Activate global listeners ONLY here
    useHardwareEvents({ enabled: true });

    const { clearTested, os, setOs } = useKeyboard();
    ...
    return (
        <div className="min-h-screen bg-skin-fill text-skin-text flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-500">
            ...
            <div className="fixed top-6 right-6 flex gap-3 z-50">
                {/* Toggle Game Mode (Navigate to /zen) */}
                <button
                    onClick={() => navigate('/zen')}
                    className="p-3 bg-skin-card border-skin-border text-skin-text hover:opacity-80 rounded-full border backdrop-blur-md transition-all shadow-sm flex items-center gap-2"
                    title="Enter Zen Mode"
                >
                    <Sparkle size={20} />
                    <span className="text-xs font-bold pr-1">ZEN</span>
                </button>
